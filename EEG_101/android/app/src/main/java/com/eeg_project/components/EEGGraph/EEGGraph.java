...
// Android View that handles plot activities
public class EEGGraph extends FrameLayout {
    ...
    public EEGGraph(Context context) {
        ...
        Log.w("EEG 101", "Started EEGGraph ");
        initView(context);
    ...
    // -----------------------------------------------------------------------
    // Lifecycle methods (init and onDetachFromWindow)
    ...
    public void initView(Context context) {
        ...
        Log.w(TAG, "Created Random Plot");
    }

    @Override
    public void onVisibilityChanged(View changedView, int visibility){
        if (visibility == View.INVISIBLE){
            Log.w(TAG, "View changed to invisible");
            stopThreads();
        }
        else{
            startDataThread();
    ...
    private class MyPlotUpdater implements Runnable {
        Plot plot;
